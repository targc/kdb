...
kubectl label node k3d-kdb-local-agent-0 "${KDB_TAINT_KEY}=${KDB_TAINT_VALUE}" --overwrite
...
kubectl label node k3d-kdb-local-agent-1 "${KDB_TAINT_KEY}=${KDB_TAINT_VALUE}" --overwrite
